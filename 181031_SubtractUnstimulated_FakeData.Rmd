...
#subtractun<-function (datatable) {
      ...
      #analytes <- c("ifng", "tnfa")
      #for (a in analytes){
      ##Subtract out unstim
      #newcolumn<-unlist(lapply(y,function(g)
        #(g[,..a]- as.matrix(subset(g[,..a], g$stim=="un"))[1,1])
      #))
      ##Create new datatable with unstim substractions applied to all donors 
      #datatable[,a] <- newcolumn}
      #datatable
      #}

 
doughnuts<- c("ifng", "tnfa")  
subtractun<-function (datatable,analytes) {
      y<- split(datatable, donor)
      for (a in analytes){
      ##Subtract out unstim
      newcolumn<-unlist(lapply(y,function(g)
...
#}
    ...
    #subtractun<-function (datatable) {
      ...
      #y<- split(datatable, donor)
      #analytes<- sapply(datatable, is.numeric)
      #for (a in analytes){
      ##Subtract out unstim
      #newcolumn<-unlist(lapply(y,function(g)
      #(g[,a]- as.matrix(subset(g[,a], g$stim=="un"))[1,1])
...
#newfake<-lapply(fake,subtractun(fake, "ifng"))
#library(knitr)
#kable(newfake)
```
